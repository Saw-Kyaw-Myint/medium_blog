    ...
    <div class="container">
        <div class="post-whole">
            ...
            <form action="{{ route('post.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="category">
                    ...
                    {{--  <select name="category[]" id="" multiple>
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}"  {{ old('category') == $category->id  ? 'selected' : ' ' }}>{{ $category->ctitle }}</option>
                        @endforeach
                    </select>  --}}
                    ...
                    <div class="multiSelect">
                        <select multiple name="category[]" class="multiSelect_field" data-placeholder="Select Category">
                            ...
                            <option value="{{ $cat->id }}" {{in_array($cat->id, old('category') ?: []) ? "selected": ""}}>{{ $cat->ctitle }}</option>
                        @endforeach
                        </select>
                      </div>
                ...
                <div class="file">
                    <input type="file" name="image" value="{{ old('image') }}" onchange="loadFile(event)">
                    @error('image')
                        <span class="error-message">{{ $message }}</span>
                    @enderror
